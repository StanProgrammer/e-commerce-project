...
Object.entries(product).forEach(([key, value]) => {
  // Always send stock — even empty, so the backend clears tracking
  if (key === "stock" || value !== "") {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-3xl bg-white shadow-xl rounded-2xl p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* SKU — read-only, auto-generated by the server */}
          <div className="rounded-lg border border-gray-200 bg-gray-50 px-4 py-3">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              SKU (auto-generated)
            </label>
            <p className="font-mono text-sm text-gray-600">
              {productData?.product?.sku || "Assigned automatically on save"}
            </p>
          </div>
